...
grep -Fq '020_document_templates_barcode_labels.sql' cmd/api/main.go || fail 'API readiness is not pinned to document-template migration'
...
grep -Fq 'CREATE TABLE document_templates' migrations/020_document_templates_barcode_labels.sql || fail 'document template table is missing'
grep -Fq 'document_template_snapshot' migrations/020_document_templates_barcode_labels.sql || fail 'sale document template snapshot is missing'
grep -Fq '/v1/document-templates' cmd/api/main.go || fail 'document template API is missing'
grep -Fq '/v1/print/label-catalog' cmd/api/main.go || fail 'label catalog API is missing'
grep -Fq 'documentTemplateSnapshot' internal/sales/service.go || fail 'new sales do not snapshot the selected/default invoice template'
grep -Fq 'DocumentTemplateID' internal/sales/model.go || fail 'sales payload cannot select an allowed print template'
grep -Fq 'document_template_id:documentTemplateId||undefined' web/lib/edge.ts || fail 'Store Edge offline queue is missing selected document template identity'
grep -Fq 'body["document_template_id"] = sale.DocumentTemplateID' internal/storeedge/cloud.go || fail 'Store Edge cloud sync is missing selected document template identity'
test -f web/app/store/documents/page.tsx || fail 'document and barcode template designer page is missing'
grep -Fq 'printEdgeLabel' web/app/store/documents/page.tsx || fail 'barcode designer is not connected to Store Edge printing'
