...
export type ReservationLite = { id:string; product_id:string; product_title:string; store_id:string; store_name:string; buyer_role?:string; sale_id?:string; paid_amount:number; due_amount:number; qty:number; unit_price:number; total_amount:number; status:string; created_at:string };
...
export async function requestReservationCharge(session:UserSession,id:string,note=""){ return request<TradeRequest>(session,`/v1/mechanic/reservations/${id}/trade-charge`,{method:"POST",body:JSON.stringify({note})}); }
