...
export default function StoreEdgePage(){
  ...
  useEffect(()=>{
    const timer=setInterval(()=>{void getEdgeLifecycleStatus().then(setLifecycle).catch(()=>setLifecycle(null))},3500);
  ...
  return <>
    <div className="page-head"><div><span className="eyebrow">تداوم فروش</span><h1>Store Edge و فروش آفلاین</h1><p>اجرای Agent، توقف، Restart و بروزرسانی این کامپیوتر از همین پنل مدیریت می‌شود؛ کاربر نیازی به Terminal یا Services ویندوز ندارد.</p></div><a className="ghost-btn" target="_blank" rel="noreferrer" href={STORE_EDGE_URL}>باز کردن صندوق آفلاین</a></div>
    {message&&<div className="network-notice">{message}</div>}

    <section className="edge-manager-card panel">
      <div className="panel-head"><div><span className="panel-kicker">Lifecycle Manager</span><h2>کنترل Store Agent این کامپیوتر</h2><p>Manager همیشه سبک و محلی باقی می‌ماند تا حتی وقتی Agent متوقف است بتوانی دوباره آن را Start یا Update کنی.</p></div>
      ...
      {managerInstalled?<>
        <div className="edge-manager-stats">
          ...
          <div><span>حالت سرویس</span><b>{lifecycle.service_mode==="windows"?"Windows Service":lifecycle.service_mode==="systemd-user"?"systemd user":"Development"}</b></div>
        </div>
        ...
        {!lifecycle.update_enabled&&<small className="edge-update-hint">کانال بروزرسانی امضاشده روی این build تنظیم نشده است. نسخه Release باید Manifest URL و Ed25519 public key را هنگام build داخل Manager قرار دهد.</small>}
    ...
    <section className="edge-policy panel"><b>سیاست Lifecycle نسخه 15.8.1</b><p><strong>Stop Agent</strong> فقط Worker فروش/سخت‌افزار را متوقف می‌کند؛ Manager روی پورت محلی خودش روشن می‌ماند، بنابراین Start/Restart/Update همیشه از همین صفحه ممکن است. بروزرسانی فقط وقتی نصب می‌شود که SHA-256 و امضای Ed25519 فایل Release معتبر باشند.</p></section>
  </>;
}
