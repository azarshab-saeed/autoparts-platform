...
const friendlyError=(e:unknown)=>{const msg=e instanceof Error?e.message:String(e||"");const balance=msg.match(/amount exceeds outstanding balance \((\d+)\)/);if(balance)return `مبلغ چک از مانده باز شخص بیشتر است. حداکثر مبلغ قابل ثبت ${money(Number(balance[1]))} تومان است.`;if(msg.includes("limit must be between 1 and 100"))return "خطای دریافت فهرست چک‌ها برطرف نشده است؛ صفحه را تازه‌سازی کن.";return msg||"عملیات ناموفق بود"};
...
export default function ChecksPage(){
  ...
  async function load(){
    ...
    const out=await getChecks(session,{direction:tab==="banks"?undefined:tab,q,status,limit:100});setChecks(out.items??[]);
  }
  useEffect(()=>{void load().catch(e=>setError(friendlyError(e)))},[session,tab,status]);
  async function search(){if(!session||tab==="banks")return;setError("");try{const out=await getChecks(session,{direction:tab,q,status,limit:100});setChecks(out.items??[])}catch(e){setError(friendlyError(e))}}
