...
export default function ChecksPage(){
  ...
  async function load(){
    ...
    const out=await getChecks(session,{direction:tab==="banks"?undefined:tab,q,status,limit:200});setChecks(out.items??[]);
  }
  useEffect(()=>{void load().catch(e=>setError(e instanceof Error?e.message:"دریافت اطلاعات چک و بانک ناموفق بود"))},[session,tab,status]);
  async function search(){if(!session||tab==="banks")return;setError("");try{const out=await getChecks(session,{direction:tab,q,status,limit:200});setChecks(out.items??[])}catch(e){setError(e instanceof Error?e.message:"جست‌وجو ناموفق بود")}}
  ...
  async function submitCheck(){
    ...
    }catch(e){setError(e instanceof Error?e.message:"ثبت چک ناموفق بود")}finally{setBusy(false)}
  }
  ...
  async function submitAction(){
    ...
    try{await transitionStoreCheck(session,action.check.id,action.action,{bankAccountId:needsBank?actionBank:undefined,supplierId:needsSupplier?actionSupplier:undefined,note:actionNote});setSuccess("وضعیت چک با ثبت حسابداری به‌روزرسانی شد.");setAction(null);await load()}catch(e){setError(e instanceof Error?e.message:"تغییر وضعیت چک ناموفق بود")}finally{setBusy(false)}
  }
