...
func (s *Service) FulfillReservation(ctx context.Context, cmd FulfillReservationCommand) (ReservationFulfillment, error) {
	...
	if _, err = tx.Exec(ctx, `INSERT INTO sales(id,tenant_id,store_id,warehouse_id,customer_id,status,total_amount,paid_amount,due_amount,idempotency_key) VALUES($1,$2,$3,$4,$5,'posted',$6,$7,$8,$9)`, saleID, cmd.TenantID, cmd.StoreID, warehouseID, cmd.CustomerID, total, paid, due, saleIdempotencyKey); err != nil {
		return ReservationFulfillment{}, err
	}
	...
	if _, err = tx.Exec(ctx, `INSERT INTO sale_items(tenant_id,sale_id,product_id,qty,unit_price,unit_cost,line_total) VALUES($1,$2,$3,$4,$5,$6,$7)`, cmd.TenantID, saleID, productID, qty, unitPrice, avgUnitCost, total); err != nil {
		return ReservationFulfillment{}, err
	}
